const villes = [
  ...
  const nomsDesVilles = villes.map(v => v.nom)
...
const habitantsYverdon = villes.find(estYverdon).population;
  ...
  .map(v => v.population)
  .reduce((result, pop) => result + pop,0); // valeur de départ 0 => result à chaque fois et pop c'est le résultat obtenu pour chaque ville
